...
メガ進化でこうげき+25・ぼうぎょ+35・とくこう+10・とくぼう+30と、攻守両面が底上げされます。すばやさは40のまま変化せず、環境上位のほとんどのポケモンに先手を取られる遅さが弱点として残ります。主流のEV配分（H32-A32-D2）はすばやさに振らないため、実戦でのすばやさ実数値はいじっぱりで60、ゆうかんで54にとどまります。
...
<strong>性格:</strong> ゆうかん（A↑ S↓。すばやさ実数値は54。性格全体ではいじっぱり82.3%・ゆうかん12.9%）<br>
...
性格にゆうかんを選んですばやさをさらに下げると、グソクムシャ同士の対面などで後攻からとんぼがえりを打ち、相手の交代先を見てから自分の交代先を選べます。性格全体ではいじっぱり（82.3%）が多数派のため、同族対面ではゆうかんの方がほぼ確実に後攻を取れます（型別の性格集計はありません。とんぼがえりを持たない型2には、後攻になる利点がありません）。
...
<div>
<strong>特性:</strong> ききかいひ（100%）→メガ進化後かたいツメ<br>
<strong>性格:</strong> いじっぱり（A↑ C↓。すばやさ実数値は60）<br>
...
つるぎのまい（採用率15.1%）でこうげきを2段階上げてからアイアンヘッド・インファイト（採用率24.9%）で押し切る構成です。インファイトは威力120と高火力ですが、使用後に自分の防御・特防が下がるため積んだ後の連打が前提になります。とんぼがえりを持たないため後攻になる利点がなく、性格はすばやさを下げないいじっぱりです。
