...
ステロなし・積みなし・素の状態で確定1発が取れる最も明確なシナリオ。ステロ受けダメージが×1×1=12.5%（19HP）と小さいため、ステロ後でもムーンフォースは確定2発のまま変わらない。
...
マルチスケイルはHP満タン時にダメージを1/2にする特性。ステロ（×2弱点=25%削り）を踏んでいないフルHPの状態では効果が残る。
...
ステロ後（マルチスケイル解除・HP149以下）：ムーンフォース最小値249 > HP149 → **両者とも確定1発**（差なし）。
...
メガギャラドス（いじっぱりAS：HP170, D150, あく×2）に対して、ムーンフォースは既に確定1発を達成しています（min 198 > HP170）。×2タイプ補正があるため、はめつのひかりを使う必要はありません。
