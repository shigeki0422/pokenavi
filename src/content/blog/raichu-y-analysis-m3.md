...
でんじほう（威力120・でんき・タイプ一致×1.5・ノーガード必中）のダメージを、おくびょうC212とひかえめC233で比較する。ダメージは最高乱数（×1.0）／最低乱数（×0.85）の範囲で表記。

| ポケモン | 使用率 | HP | おくびょう（最高/最低） | ひかえめ（最高/最低） | 判定 |
|---|---|---|---|---|---|
| バシャーモ | 16位 | 157 | 169 / 144 | 186 / 158 | ★ 乱数1発 → 確定1発 |
| サーフゴー | 15位 | 164 | 153 / 130 | 168 / 142 | 確定2発 → 乱数1発 |
| メタグロス | 10位 | 157 | 132 / 112 | 144 / 122 | 確定2発（変化なし） |

**バシャーモ（16位）** が最も影響の大きい相手。おくびょう型は最高乱数169がHP157を超えるが、最低乱数144では倒しきれず乱数1発になる。ひかえめ型は最低乱数158がHP157を上回るため確定1発。ノーガードで外れないでんじほうでも乱数が絡む場面を、ひかえめは解消できる。

サーフゴー（15位）はひかえめでも乱数1発止まり（最高乱数168はHP164を超えるが最低乱数142では超えない）。こちらはどちらの型を選んでも確定1発にはならない。
