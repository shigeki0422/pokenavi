...
pub const N_ITEM_FLAGS: usize = 18;   // v2: 持ち物カテゴリ 8→18（features.py と 1:1）
...
// simulator/features.py の _ITEM_FLAGS（FEAT_V2 適用後）と 1:1。順序も一致させること。
// M-6 上位50種の非メガ持ち物使用率のうち 29% が旧8カテゴリでは1bitも立たず、
// ネットからは「無持ち物」と同じに見えていた（識別率 71%→99.7%）。
const ITEM_FLAGS: [&[&str]; N_ITEM_FLAGS] = [
    &["きあいのタスキ"],
    &["たべのこし"],
    &["オボンのみ"],
    &["ラムのみ", "カゴのみ"],
    // 半減きのみ（v2で取りこぼしを合流。次元は増えない）
    &[
        "シュカのみ", "ハバンのみ", "ソクノのみ", "ヨロギのみ",
        "ヨプのみ", "リンドのみ", "カシブのみ", "リリバのみ", "イトケのみ", "オッカのみ",
        "ビアーのみ", "バンジのみ", "ウタンのみ", "タンガのみ", "ナモのみ", "ホズのみ",
        "ロゼルのみ", "シャンプのみ", "ヤチェのみ",
    ],
    // タイプ強化（v2で きせきのタネ/グランドコート を合流）
    &[
        "しんぴのしずく", "じしゃく", "とけないこおり", "りゅうのキバ", "やわらかいすな",
        "するどいくちばし", "くろいメガネ", "まがったスプーン", "ようせいのハネ",
        "メタルコート", "のろいのおふだ", "きせきのタネ", "グランドコート",
    ],
    &["こだわりスカーフ"],
    &["ひかりのこな"],
    // ── v2 追加10カテゴリ（実測で寄与の大きい順） ──
    &["いのちのたま"],                                                   // 反動火力 266%pt
    &["ひかりのねんど"],                                                 // 画面延長 205
    &["ゴツゴツメット"],                                                 // 接触ダメージ返し 199
    &["ふうせん"],                                                       // じめん無効 85
    &["しめったいわ", "あついいわ", "さらさらいわ", "つめたいいわ"],       // 天候延長 57
    &["サイコシード", "グラスシード", "エレキシード", "ミストシード"],     // フィールド登場強化 70
    &["ノーマルジュエル", "たつじんのおび"],                              // 初撃/抜群強化 42
    &["しろいハーブ"],                                                   // 能力低下リセット 22
    &["レッドカード", "だっしゅつボタン"],                                // 強制交代 10
    &["ピントレンズ", "するどいツメ"],                                    // 急所率上昇
];
