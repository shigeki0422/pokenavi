...
for name, type_, cat, power, accuracy, pp, effect in moves:
    ...
    # ── リチャージ技 ──
    elif name in ('はかいこうせん', 'ギガインパクト', 'ブラストバーン', 'ハードプラント', 'ハイドロカノン', 'がんせきほう', 'スターアサルト'):
    ...
    if name in ('アシストパワー', 'つけあがる'):
        ...
        tests_for_move.append('# くらいつく: 相手と自分の両方が交代不可(トラップ)になる')
        tests_for_move.append('_pjl = make_poke(type1="あく", atk_b=120); _djl = make_poke(type1="エスパー", hp_b=255, def_b=120)')
        tests_for_move.append('execute(_pjl, _djl, "くらいつく")')
        tests_for_move.append('check("相手が交代不可: くらいつく", _djl.trapped is True)')
        tests_for_move.append('check("自分も交代不可: くらいつく", _pjl.trapped is True)')
        tests_for_move.append('_pjl2 = make_poke(type1="あく", atk_b=120); _djl2 = make_poke(type1="エスパー", hp_b=255, def_b=120)')
        tests_for_move.append('execute(_pjl2, _djl2, "かみくだく")')
        tests_for_move.append('check("他の噛み技では交代不可にならない(負例): くらいつく", _djl2.trapped is False and _pjl2.trapped is False)')
    elif name == 'たこがため':
        tests_for_move.append('# たこがため: 相手が交代不可になり、ターン終わりにB/Dが1段階下がる')
        tests_for_move.append('from simulator.battle import Battle as _Bol')
        tests_for_move.append('_pol = make_poke(type1="かくとう"); _dol = make_poke(type1="ノーマル", hp_b=255)')
        tests_for_move.append('execute(_pol, _dol, "たこがため")')
        tests_for_move.append('check("相手が交代不可: たこがため", _dol.trapped is True)')
        tests_for_move.append('check("使用時点ではB/D不変: たこがため", _dol.stage_defense == 0 and _dol.stage_sp_defense == 0)')
        tests_for_move.append('_Bol(BattleSide([_dol]), BattleSide([make_poke()]))._end_of_turn()')
        tests_for_move.append('check("ターン終わりにB/D-1: たこがため", _dol.stage_defense == -1 and _dol.stage_sp_defense == -1, f"B={_dol.stage_defense} D={_dol.stage_sp_defense}")')
        tests_for_move.append('_dfree = make_poke(type1="ノーマル", hp_b=255)')
        tests_for_move.append('_Bol(BattleSide([_dfree]), BattleSide([make_poke()]))._end_of_turn()')
        tests_for_move.append('check("非たこがためは下がらない(負例): たこがため", _dfree.stage_defense == 0 and _dfree.stage_sp_defense == 0)')
    elif name == 'きょけんとつげき':
        tests_for_move.append('# きょけんとつげき: 使用後は無防備状態（被ダメ2倍・必中）、次に自分が行動すると解除')
        tests_for_move.append('_pgv = make_poke(type1="ドラゴン", atk_b=120, hp_b=255, def_b=100)')
        tests_for_move.append('_dgv = make_poke(type1="ノーマル", hp_b=255, def_b=100, atk_b=100)')
        tests_for_move.append('execute(_pgv, _dgv, "きょけんとつげき")')
        tests_for_move.append('check("無防備状態付与: きょけんとつげき", getattr(_pgv, "_defenseless", False) is True)')
        tests_for_move.append('_pgv0 = make_poke(type1="ドラゴン", atk_b=120, hp_b=255, def_b=100)')
        tests_for_move.append('_dv_v = dmg(_dgv, _pgv, "たいあたり"); _dv_n = dmg(_dgv, _pgv0, "たいあたり")')
        tests_for_move.append('check("無防備で被ダメ2倍: きょけんとつげき", _dv_v == _dv_n * 2, f"vuln={_dv_v} normal={_dv_n}")')
        tests_for_move.append('from simulator.damage import check_hit as _chgv')
        tests_for_move.append('check("無防備で必中: きょけんとつげき", all(_chgv(_dgv, _pgv, dl.get_move("ふぶき"), BattleField()) for _ in range(20)))')
        tests_for_move.append('execute(_pgv, _dgv, "たいあたり")')
        tests_for_move.append('check("行動で無防備解除: きょけんとつげき", getattr(_pgv, "_defenseless", False) is False)')
    elif name == 'でんこうそうげき':
        tests_for_move.append('# でんこうそうげき: 非でんきは失敗、でんきなら攻撃後に自分のでんきが消える')
        tests_for_move.append('_pdf = make_poke(type1="ノーマル", atk_b=120); _ddf = make_poke(type1="ノーマル", hp_b=255, def_b=150)')
        tests_for_move.append('_hpd1 = _ddf.hp; execute(_pdf, _ddf, "でんこうそうげき")')
        tests_for_move.append('check("非でんき失敗: でんこうそうげき", _ddf.hp == _hpd1, f"hp={_ddf.hp}/{_hpd1}")')
        tests_for_move.append('_pds = make_poke(type1="でんき", type2="ひこう", atk_b=120); _dds = make_poke(type1="ノーマル", hp_b=255, def_b=150)')
        tests_for_move.append('_hpd2 = _dds.hp; execute(_pds, _dds, "でんこうそうげき")')
        tests_for_move.append('check("でんきなら成功: でんこうそうげき", _dds.hp < _hpd2, f"hp={_dds.hp}/{_hpd2}")')
        tests_for_move.append('check("でんきタイプ消失: でんこうそうげき", "でんき" not in (_pds.type1, _pds.type2), f"types={_pds.type1}/{_pds.type2}")')
    elif name == 'かえんボール':
        tests_for_move.append('# かえんボール: 使うと自分のこおり状態を治す')
        tests_for_move.append('_pfb = make_poke(type1="ほのお", atk_b=100); _pfb.status = "freeze"')
        tests_for_move.append('execute(_pfb, make_poke(type1="くさ", hp_b=255, def_b=150), "かえんボール")')
        tests_for_move.append('check("自分こおり治癒: かえんボール", _pfb.status != "freeze", f"status={_pfb.status}")')
        tests_for_move.append('_pfb2 = make_poke(type1="ほのお", atk_b=100); _pfb2.status = "burn"')
        tests_for_move.append('execute(_pfb2, make_poke(type1="くさ", hp_b=255, def_b=150), "かえんボール")')
        tests_for_move.append('check("こおり以外は治らない(負例): かえんボール", _pfb2.status == "burn", f"status={_pfb2.status}")')
    elif name == 'コートチェンジ':
        tests_for_move.append('# コートチェンジ: 自分側・相手側の場が双方向に入れ替わる（設置物を実際に踏ませて確認）')
        tests_for_move.append('from simulator.battle import _entry_effects as _eecc')
        tests_for_move.append('_fcc = BattleField()')
        tests_for_move.append('_pcc1 = make_poke(hp_b=255); _pcc2 = make_poke(hp_b=255)')
        tests_for_move.append('_scc1 = BattleSide([_pcc1]); _scc2 = BattleSide([_pcc2])')
        tests_for_move.append('_scc1.field_idx, _scc2.field_idx = 0, 1')
        tests_for_move.append('_fcc.stealth_rock[0] = True; _fcc.spikes[1] = 3')
        tests_for_move.append("_execute_move(_scc1, _scc2, Action(type='move', move=dl.get_move('コートチェンジ')), _fcc)")
        tests_for_move.append('_eecc(_pcc1, 0, _fcc, _pcc2); _eecc(_pcc2, 1, _fcc, _pcc1)')
        tests_for_move.append('_lcc1 = _pcc1.max_hp - _pcc1.hp; _lcc2 = _pcc2.max_hp - _pcc2.hp')
        tests_for_move.append('check("双方向入替(自分の場↔相手の場): コートチェンジ", _lcc1 == _pcc1.max_hp // 4 and _lcc2 == _pcc2.max_hp // 8, f"self_lost={_lcc1}(まきびし3段1/4期待) opp_lost={_lcc2}(ステロ1/8期待)")')
        tests_for_move.append('check("フラグも入れ替わる: コートチェンジ", _fcc.stealth_rock == [False, True] and _fcc.spikes == [3, 0], f"sr={_fcc.stealth_rock} spk={_fcc.spikes}")')
        tests_for_move.append('_fcc2 = BattleField(); _scd1 = BattleSide([make_poke()]); _scd2 = BattleSide([make_poke()])')
        tests_for_move.append('_scd1.field_idx, _scd2.field_idx = 0, 1')
        tests_for_move.append('_scd1.reflect = True; _scd1.reflect_count = 4; _scd2.tailwind = True; _scd2.tailwind_count = 3')
        tests_for_move.append("_execute_move(_scd1, _scd2, Action(type='move', move=dl.get_move('コートチェンジ')), _fcc2)")
        tests_for_move.append('check("壁・おいかぜも双方向: コートチェンジ", _scd1.reflect is False and _scd2.reflect is True and _scd2.reflect_count == 4 and _scd1.tailwind is True and _scd1.tailwind_count == 3 and _scd2.tailwind is False, f"refl={_scd1.reflect}/{_scd2.reflect} tw={_scd1.tailwind}/{_scd2.tailwind}")')
    elif name == 'さいきのいのり':
        tests_for_move.append('# さいきのいのり: ひんしを最大HPの1/2で復活。ひんしがいなければ失敗')
        tests_for_move.append('_prb = make_poke(type1="ノーマル"); _drb = make_poke(hp_b=100)')
        tests_for_move.append('_drb.hp = 0; _drb.is_alive = False')
        tests_for_move.append("_execute_move(BattleSide([_prb, _drb]), BattleSide([make_poke()]), Action(type='move', move=dl.get_move('さいきのいのり')), BattleField())")
        tests_for_move.append('check("ひんし復活: さいきのいのり", _drb.is_alive is True)')
        tests_for_move.append('check("復活HPは1/2: さいきのいのり", _drb.hp == _drb.max_hp // 2, f"hp={_drb.hp}/{_drb.max_hp}")')
        tests_for_move.append("_lrb = _execute_move(BattleSide([make_poke(type1='ノーマル'), make_poke()]), BattleSide([make_poke()]), Action(type='move', move=dl.get_move('さいきのいのり')), BattleField())")
        tests_for_move.append('check("ひんし無しは失敗(負例): さいきのいのり", any("失敗" in _l for _l in _lrb))')
    elif name == 'ねらいうち':
        tests_for_move.append('# ねらいうち: 引き受け無視は1v1では効果なし（急所+1は急所テスト側で検証）')
        tests_for_move.append('_psn = make_poke(type1="みず", spatk_b=100); _dsn = make_poke(type1="ノーマル", hp_b=255, spdef_b=100)')
        tests_for_move.append('_hpsn = _dsn.hp; execute(_psn, _dsn, "ねらいうち")')
        tests_for_move.append('check("引き受け無視は1v1で無影響(通常通り命中): ねらいうち", _dsn.hp < _hpsn, f"hp={_dsn.hp}/{_hpsn}")')
    elif name == 'アイアンローラー':
