...
DETECTORS = [
    ...
    (r'必ず命中する', ['必中']),
    (r'必ず急所', ['急所率', 'crit_chance', '必ず急所(1.0)']),
    (r'HPに\d+ダメージ', ['固定']),
    ...
    (r'自分のでんきタイプがなくなる', ['でんこうそうげき','タイプ']),
    (r'相手と自分をくらいつく状態にする', ['くらいつく','交代不可']),
    (r'相手をにげられない状態とたこがため状態にする', ['たこがため','交代不可']),
    (r'自分がでんきタイプではない場合、失敗する', ['でんこうそうげき','失敗']),
    (r'次に自分が行動するまで、自分は無防備状態', ['無防備']),
    (r'使うと自分のこおり状態をなおす', ['こおり']),
    (r'味方と相手の場の状態を入れ替える', ['コートチェンジ','場']),
    (r'手持ちのひんしのポケモンを最大HPの1/2の状態で復活させる', ['復活','1/2']),
    (r'技を引き受ける特性や技の影響を無視する', ['引き受け']),
