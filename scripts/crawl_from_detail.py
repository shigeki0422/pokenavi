"""
詳細画面から直接再開するスクリプト。高速ナビゲーションなし。
現在の画面状態: CURRENT_RANK の詳細画面を表示中。
BACKを押してリスト画面に戻り、CURRENT_RANK から詳細クロールを開始する。
"""
...
CURRENT_RANK  = 91   # 現在詳細画面で表示中のrank（BACKで戻ってここから開始）
END_RANK      = 200
...
def main():
    ...
    log(f"現在 {CURRENT_RANK}位の詳細画面 → BACKでリスト画面に戻る...")
    tap(*BACK_BTN, wait=2.0)

    log(f"{CURRENT_RANK}位〜{END_RANK}位の詳細クロール開始")
    for rank in range(CURRENT_RANK, END_RANK + 1):
    ...
    log(f"完了: {CURRENT_RANK}〜{END_RANK}位")
