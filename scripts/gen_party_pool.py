...
def _ev_clean(s):
    """ブッパ型か：最大振り(≥28)が1〜2ステ、中間値(7〜27)なし、端数(1〜6)は最大2ステまで。
    ＝実質「2ステに全振り＋余り」。散らばった振り(H32 A18 B14 D2 等)は弾く。

    端数を1ステまでにしていたが、EV上限32・予算66では「A全振り＋S全振り＋余り2」が
    自然に H1/B1 の2ステに乗るため、最も一般的な振り方 1/32/1/0/0/32 を構造的に排除していた。
    実測 M-6 188種のうち64種(34%)で最頻EV配分が弾かれ、ミミッキュでは使用率76%の
    いのちのたま型がプールから消えて提案294件中284件(97%)が使用率5%のピントレンズ型になっていた。
    2ステまで許すと最頻EVが通る種は124→153、使用率加重カバー率は60.0%→72.7%。"""
    ...
    return mid == 0 and 1 <= big <= 2 and small <= 2
