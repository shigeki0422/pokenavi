...
def _alive_dead(side):
    ...
    for p in side.party:
        if p.is_alive:
            boosts = sum(getattr(p, a, 0) for a in
                         ("stage_attack", "stage_sp_attack", "stage_speed", "stage_defense", "stage_sp_defense"))
            alive.append((p.name, boosts))
    dead = [p.name for p in side.party if not p.is_alive]
    return alive, dead


def _swap_p1p2(line):
    return line.replace("P1", "\x00").replace("P2", "P1").replace("\x00", "P2")


def _mirror_turn(t):
    out = {"turn": t["turn"], "logs": [_swap_p1p2(l) for l in t["logs"]],
           "side1": t["side2"], "side2": t["side1"],
           "weather": t["weather"], "weather_count": t["weather_count"],
           "trick_room": t["trick_room"], "trick_room_count": t["trick_room_count"]}
    if t.get("expl2") is not None: out["expl1"] = t["expl2"]
    if t.get("expl1") is not None: out["expl2"] = t["expl1"]
    return out
...
def play_and_record_both(specs1, specs2, season="M-2", det=8, sel_temp=0.6, seed=0, mcts_sims=None):
    ...
    def cb(b):
        nl = b.logs[prev[0]:]; prev[0] = len(b.logs)
        t = {"turn": b.turn, "logs": nl,
             "side1": side_snapshot(b.side1, b.field), "side2": side_snapshot(b.side2, b.field),
             "weather": b.field.weather, "weather_count": getattr(b.field, "weather_count", 0),
             "trick_room": b.field.trick_room,
             "trick_room_count": getattr(b.field, "trick_room_count", 0)}
        if getattr(_a1, "last_decision", None) is not None:
            t["expl1"] = _a1.last_decision; _a1.last_decision = None
        if getattr(_a2, "last_decision", None) is not None:
            t["expl2"] = _a2.last_decision; _a2.last_decision = None
        turns.append(t)
    ...
    if not s1.has_alive():
        ...
        result = 1
    n1 = [p.name for p in sel1]; n2 = [p.name for p in sel2]
    aliveA, deadA = _alive_dead(s2)   # A=P1視点：相手=s2
    aliveB, deadB = _alive_dead(s1)   # B=P2視点：相手=s1
    w1 = (f"P1 ({n1[0]}側)" if result == 1 else f"P2 ({n2[0]}側)" if result == 2 else "引き分け")
    rec1 = {"selected1": n1, "selected2": n2, "turns": turns, "result": result,
            "winner": w1, "opp_alive": aliveA, "own_dead": deadA}
    rB = {1: 2, 2: 1, 0: 0}[result]
    w2 = (f"P1 ({n2[0]}側)" if rB == 1 else f"P2 ({n1[0]}側)" if rB == 2 else "引き分け")
    rec2 = {"selected1": n2, "selected2": n1, "turns": [_mirror_turn(t) for t in turns],
            "result": rB, "winner": w2, "opp_alive": aliveB, "own_dead": deadB}
    return rec1, rec2
