"""上位環境ランカー総当たり(f1_cache)を、本番(/sim-data)とは別の検証用静的データへ書き出す。
export_f1_static のヘルパ(compute_faults/compute_vs_pokemon/_replay/explain_matchup/export_icons)を再利用し、
META+subjects を全て public/sim-data-verify/ に同梱配置（localhostで simulator?verify=1 が読む）。本番ページは無変更。
"""
import glob, json, os, sqlite3
import export_f1_static as E

SEASON = E.SEASON   # M-2（本番ランクf1と同一）
CACHE_DIR = os.path.join(os.path.dirname(__file__), "f1_cache")
OUT_DIR = os.path.join(os.path.dirname(__file__), "..", "public", "sim-data-verify")
...
def main():
    ...
    print(f"完了 → {OUT_DIR} / subjects {len(files)}本 計{sz/1e6:.1f}MB  ai_ver={ai_ver}")
