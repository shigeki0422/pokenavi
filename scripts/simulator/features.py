...
# 特徴量v2（FEAT_V2=1）: M-6 TOP50 の非メガ持ち物使用率のうち 29%(1,022%pt) が上の8カテゴリで
# 1bitも立たず、ネットからは「無持ち物」と同じに見えていた。実測で寄与の大きい順に10カテゴリ追加する。
# 既定OFF＝905次元のまま（本番ネット az_net_np.json との互換を保つ）。ONで 905→965。
_ITEM_FLAGS_V2 = [
    {"いのちのたま"},                                                    # 反動火力 266%pt
    {"ひかりのねんど"},                                                  # 画面延長 205
...
# 既存カテゴリの取りこぼし（次元は増えない。v2でのみ合流させる）
_TYPE_BOOST_V2 = {"きせきのタネ", "グランドコート"}                       # →カテゴリ5(タイプ強化)
_HALF_BERRY_V2 = {"ヨプのみ", "リンドのみ", "カシブのみ", "リリバのみ",
...
if FEAT_V2:
    _ITEM_FLAGS = [f | _TYPE_BOOST_V2 if "しんぴのしずく" in f else
                   ...
                   for f in _ITEM_FLAGS]
    _ITEM_FLAGS = _ITEM_FLAGS + _ITEM_FLAGS_V2
