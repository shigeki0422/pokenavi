...
def generate(cfg):
    ...
    draw.text((LX + 14, LY + 9), "シーズン M-4", font=get_font(18, bold=True), fill=BADGE_TXT)
    draw.text((LX + 2, LY + 48), "2026/07/14 〜", font=get_font(12), fill=META_GRAY)
...
def main():
    ...
    for md_file in sorted(BLOG.glob("*-analysis-m4.md")):
        slug = md_file.stem.replace("-analysis-m4", "")
        ...
        cfg = dict(out=f"hero-{slug}-m4.png", **palette, left_lines=[line1, line2, line3], entries=entries)
        try:
            saved = generate(cfg)
