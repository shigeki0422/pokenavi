...
- **そのポケモン自体の強み・弱み（種族値・弱点タイプ・特性など、どの型でも共通する性質）を型の強み・弱みに混ぜない**。例：「ほのお技1発で即戦闘不能」「A150の高火力」「テクニシャン+バレットパンチが強い」などはハッサム全体の話であって、HA+B型／HA+S型／HA+D型のどれを選んでも同じ。型の強み・弱みの欄には**他型と比べて何が違うか**だけを書く（余り2の振り先・S投資の有無・D投資の有無・メガ進化するか否か等）。種族値・特性・弱点の話は「基本スペック」「なぜ強いか」セクションで扱う
