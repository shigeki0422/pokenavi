...
- **S・C等の実数値は種族値・EV・性格補正から実際に計算した値を書く**。憶測値・概算値で書かない。計算式は Lv50・個体値31・EV最大32で `((種族値×2+31+63)×50÷100+5)×性格補正` （性格補正は↑1.1 / ↓0.9 / 無補正1.0）。例：
- **メガポケモンを論じる記事で素早さ・火力を比較するときは、相手もメガ後の種族値で比較する**。メガ進化前提のポケモンを論じているのに、比較対象だけ無メガ状態で書くのはアンフェアでミスリーディング。比較対象のメガ石採用率（`pokemon_items`）を確認し、メガ運用が主流ならメガ後種族値で実数値を計算する。例：メガリザードンY（メガ後S100→S実数値167）の比較対象として、ミミロップ（無メガS105）でなくメガミミロップ（メガ後S135→S実数値205）、メガフラエッテ採用率97.4%なら素体S92でなくメガ後S102→S実数値169を使う。**「メガで論じておきながら相手が無メガで脅威評価が小さく見える」誘導は禁止**
  ...
  記事内のS実数値・C実数値はすべてPythonで `python3 -c "print(int((((種族値*2+31+63)*50)//100+5)*補正))"` で検算してから書く
...
- **未実装の持ち物に言及しない**。チャンピオンズに実装済みの持ち物は `item_master`（実際に環境採用されているのは `pokemon_items` の DISTINCT＝55種）で確認できる。よくある未実装の例：**とつげきチョッキ・こだわりハチマキ・こだわりメガネ・いのちのたま・ゴツゴツメット・くろいヘドロ・あつぞこブーツ・だっしゅつボタン/パック・レッドカード**（こだわり系で実装されているのは**こだわりスカーフのみ**）。「はたきおとす」「トリック」等で持ち物に触れる際は、実在する持ち物（オボンのみ・たべのこし・きあいのタスキ・こだわりスカーフ・各種半減きのみ等）のみを挙げる
...
- **苦手なポケモンをパートナー推奨する場合は役割を明確化**: 同じポケモンが「苦手」と「パートナー」の両方に登場する場合、なぜ同居するかの文脈（役割の棲み分け・選出を分散）を明記する
